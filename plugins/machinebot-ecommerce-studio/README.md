...
**버전 1.16.6** — 시작 질의응답으로 필요한 제작 타입을 추천하고, 선택한 범위와 제품 자료를 기존 제작 절차에 연결합니다. 제품 누끼 독립 스킬과 원본·자산 기록을 포함하며, 사진으로 보이지 않는 제품 가치를 CG 판단 카탈로그로 연출·효용·상세 흐름에 연결합니다.
...
예: “누끼 한 장만”은 제품 원본과 투명 PNG 조건을 확인해 제작하고 그 한 장으로 끝냅니다. 요청하지 않은 연출·상세페이지를 자동 생성하지 않습니다. 이미 제공한 정보와 선택한 범위는 다음 제작에 이어 씁니다.
